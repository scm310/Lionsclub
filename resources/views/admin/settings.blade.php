    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('admin.settings.storeAccountNames') }}" method="POST">
                ...
                @if ($errors->has('account_names'))
                    <div class="text-danger">
                        {{ $errors->first('account_names') }}
                    </div>
                @endif
        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.settings.storedistrict') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="parent_district_id" name="parent_district_id" required>
                ...
                <div id="districtNamesContainer">
                    <div class="form-group">
                        <label for="name">District Name</label>
                        <input type="text" class="form-control district-name" name="name[]" placeholder="Enter District Name" required>
                        @error('name.*')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <button type="submit" class="btn custom-btn mt-3" style="color: white;">Save</button>
            </form>
        </div>
    </div>
</div>
            ...
            <form action="{{ route('admin.settings.storeMembershipTypes') }}" method="POST">
                ...
                <div id="membershipTypesContainer">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control mb-2 membership-type" name="membership_types[]" placeholder="Enter Membership Type" required>
                        @error('membership_types.*')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                </div>
...
<script>
    document.getElementById('addMembershipTypeBtn').addEventListener('click', function () {
        let container = document.getElementById('membershipTypesContainer');
        let inputFields = document.querySelectorAll('.membership-type');
        
        let newInput = document.createElement('input');
        newInput.type = 'text';
        newInput.name = 'membership_types[]';
        newInput.className = 'form-control mb-2 membership-type';
        newInput.placeholder = 'Enter Membership Type';
        newInput.required = true;

        // Prevent duplicate input values on the frontend
        newInput.addEventListener('blur', function () {
            let values = Array.from(inputFields).map(input => input.value.trim().toLowerCase());
            if (values.includes(newInput.value.trim().toLowerCase())) {
...
<script>
    document.getElementById('addDistrictNameBtn').addEventListener('click', function () {
        let container = document.getElementById('districtNamesContainer');
        let inputFields = document.querySelectorAll('.district-name');

        let newInput = document.createElement('input');
        newInput.type = 'text';
        newInput.name = 'name[]';
        newInput.className = 'form-control mb-2 district-name';
        newInput.placeholder = 'Enter District Name';
        newInput.required = true;
