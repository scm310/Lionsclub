...
'spouseName' => ucwords(strtolower($member->spouse_name ?? '')),                            'dob' => $member->dob ? \Carbon\Carbon::parse($member->dob)->format('d M Y') : '',
...
'accountName' => ucwords(strtolower($member->account->chapter_name ?? '')),
                         ...
                         style="cursor:pointer; background-color: #ffffff; background-image: linear-gradient(315deg, #ffffff 0%, #1e90ff 74%); color: #fff; border-radius: 10px; height: 210px;">
...
<h6 class="card-title" style="font-size: 12px; padding:4px;" title="{{ Str::title($fullName) }}">
    {{ Str::title($displayName) }}
</h6>


                            <p class="" style="font-size: 11px;"><strong>ID:</strong> {{ $member->member_id }}</p>

                            <p class="" style="font-size: 10px;" title="{{ $member->account->chapter_name ?? 'N/A' }}">
    <strong>
        {{ isset($member->account->chapter_name) ? Str::limit(Str::title($member->account->chapter_name), 15) : 'N/A' }}
    </strong>
</p>
                            <p class="" style="font-size: 10px;"><i class="fa fa-phone"></i> {{ $member->phone_number ?? 'N/A' }}</p>
                        </div>
                    </div>
                </div>
            @endforeach
